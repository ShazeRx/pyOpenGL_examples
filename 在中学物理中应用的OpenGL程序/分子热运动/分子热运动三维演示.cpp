...
void display(void) { 
   glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);//如果这一条被注释，那么前一帧的图像不会被清楚
	...
	if(setMode == 0)
	{
		...
		drawString("Press e use open box");//这里显示文字
		glRasterPos3f(xRot-0.3f,yRot-0.0f,0.0f);
		drawString("Press r use close box");
		glRasterPos3f(xRot-0.3f,yRot-0.3f,0.0f);
		drawString("Press t use flow box");
	}//欢迎界面，选择封闭模式
	if(setMode > 0)
	{
		if(numMax == 0)
	...
	if(numMax>0)
	{
		...
		{	
			...
			glEnd();//绘制框框
			glColor3ub(255, 0, 255);
			sprintf(show_1,"collision=%d",cols);
			glRasterPos3f(0.5f,-0.1f,0.0f);
			drawString(show_1);
			sprintf(show_1,"A=%d",ASurface);
			glRasterPos3f(0.5f,1.1f,0.5f);
			drawString(show_1);
			sprintf(show_1,"B=%d",BSurface);
			glRasterPos3f(1.1f,0.5f,0.5f);
			drawString(show_1);
			sprintf(show_1,"C=%d",CSurface);
			glRasterPos3f(0.5f,0.5f,-0.1f);
			drawString(show_1);
			sprintf(show_1,"D=%d",DSurface);
			glRasterPos3f(-0.1f,0.5f,0.5f);
			drawString(show_1);
			sprintf(show_1,"E=%d",ESurface);
			glRasterPos3f(0.5f,0.5f,1.1f);
			drawString(show_1);
			sprintf(show_1,"F=%d",FSurface);
			glRasterPos3f(0.5f,-0.1f,0.5f);
			drawString(show_1);
		}//封闭模式下绘制框框和显示碰撞次数
		if(setMode==3)
		{
		...
		for(i=0;i<numMax;i++)
		{
			...
			x_rand=(rand()/(double)(RAND_MAX)-0.5f)/20;
			*(xCoord+i)+=(*(xSpeed+i)*multi);
			*(xSpeed+i)+=x_rand;
			if(setMode==2||setMode==3)
			{
				if(*(xCoord+i)>1.0f){if(setMode==2){*(xCoord+i)=1.0f;*(xSpeed+i)=0.0f-*(xSpeed+i);}cols++;BSurface++;if(setMode==3)*(xCoord+i)-=1.0f;}
				if(*(xCoord+i)<0.0f){if(setMode==2){*(xCoord+i)=0.0f;*(xSpeed+i)=0.0f-*(xSpeed+i);}cols++;DSurface++;if(setMode==3)*(xCoord+i)+=1.0f;}
			}
			x_rand=(rand()/(double)(RAND_MAX)-0.5f)/20;
			...
			*(ySpeed+i)+=x_rand;
			if(setMode==2||setMode==3)
			{
				if(*(yCoord+i)>1.0f){if(setMode==2){*(yCoord+i)=1.0f;*(ySpeed+i)=0.0f-*(ySpeed+i);}cols++;ASurface++;if(setMode==3)*(yCoord+i)-=1.0f;}
				if(*(yCoord+i)<0.0f){if(setMode==2){*(yCoord+i)=0.0f;*(ySpeed+i)=0.0f-*(ySpeed+i);}cols++;FSurface++;if(setMode==3)*(yCoord+i)+=1.0f;}
			}
			x_rand=(rand()/(double)(RAND_MAX)-0.5f)/20;
			...
			*(zSpeed+i)+=x_rand;
			if(setMode==2||setMode==3)
			{
				if(*(zCoord+i)>1.0f){if(setMode==2){*(zCoord+i)=1.0f;*(zSpeed+i)=0.0f-*(zSpeed+i);}cols++;ESurface++;if(setMode==3)*(zCoord+i)-=1.0f;}
				if(*(zCoord+i)<0.0f){if(setMode==2){*(zCoord+i)=0.0f;*(zSpeed+i)=0.0f-*(zSpeed+i);}cols++;CSurface++;if(setMode==3)*(zCoord+i)+=1.0f;}
			}
		}
	}
	glPopMatrix();
...
void keyboard (unsigned char key, int x, int y) {
      switch (key) {
	   ...
	   case 'R': if(setMode==0)setMode=2;break;
	   case 't': if(setMode==0)setMode=3;break;
	   case 'T': if(setMode==0)setMode=3;break;
...
int main(int argc, char *argv[]) { 
   ...
   if (glutCreateWindow ("分子动理论三维演示") == GL_FALSE)
      exit (1);
	ViewerN::init(); 
